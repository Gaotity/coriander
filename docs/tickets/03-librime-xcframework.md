...
- **Erratum (fixed in ticket 04):** the first build of this script referenced `lib/librime-static.a`, but `rime-static`'s OUTPUT_NAME is `rime` — and macOS `libtool -static` only warns (exit 0) on missing inputs, so the first XCFramework silently contained only the deps. Fixed: merge `lib/librime.a` and hard-verify `_rime_get_api` in the merged archive.
